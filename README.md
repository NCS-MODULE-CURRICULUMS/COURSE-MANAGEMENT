...
그래서 **로컬 서버로 열었을 때만** 원문을 봅니다.

```
python COURSE-MANAGEMENT/tools/pdf-server.py
http://127.0.0.1:8765/ 로 들어간다   →  학습모듈 칸에 [PDF] 단추가 생긴다
```

배포된 `github.io` 주소에서는 **학습모듈 칸을 비워 둡니다(`—`).** 눌러도 안 되는
단추를 두지 않기 위해서입니다. 표 아래 집계의 `학습모듈 N건` 으로 확보 여부는
그대로 알 수 있습니다.

단추는 정적 HTML 에 없습니다. `pdfview.js` 가 주소가 로컬일 때만
`[data-lm]` 칸을 단추로 바꿉니다. 그래서 배포본에는 누를 것이 아예 없습니다.

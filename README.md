...
세분류 표의 **PDF** 단추가 한국직업능력연구원 학습모듈 원문을 모달로 펼칩니다.

원문은 **이 저장소에 없습니다.** NCS 저작권정책이 학습모듈을 **공공누리 제2유형**
(출처표시 · 상업적 이용 금지)으로 두고, 지적재산권 고지에 이렇게 적혀 있기 때문입니다.

> NCS 학습모듈에는 국가(교육부)가 저작재산권 일체를 보유하지 않은 저작물들(출처가
> 표기되어 있는 도표, 사진, 삽화, 도면 등)이 포함되어 있으므로 이러한 저작물들의
> 변형, 복제, 공연, **배포**, **공중 송신** … 을 위해서는 원작자의 동의를 받아야 합니다.

공개 사이트에 올리는 것이 곧 배포·공중송신이라 하지 않습니다.
크기로도 불가능합니다 — 291개 **3.0GB**(최대 116MB), Pages 한도는 1GB 입니다.
원격 링크도 막혀 있습니다: `ncs.go.kr` 내려받기는 `Content-Disposition: attachment` 에
CORS 헤더가 없어 iframe 으로도 pdf.js 로도 띄울 수 없습니다(헤더 직접 확인).

그래서 **강사 PC 의 파일을 브라우저가 직접 엽니다.** 두 갈래이고, 뷰어가 알아서 고릅니다.

```
1. 로컬 서버로 사이트째 열기  →  아무것도 묻지 않고 바로 열림  ← 권함
     python COURSE-MANAGEMENT/tools/pdf-server.py
     그리고 http://127.0.0.1:8765/ 로 들어간다
2. 배포된 github.io 에서 볼 때 (Chrome/Edge)
     처음 한 번      →  커리큘럼 저장소들을 담은 상위 폴더를 고름
     그 다음부터     →  기억해 둔 폴더에서 열림
     브라우저 재시작  →  [허용] 한 번 (폴더는 그대로 기억)
3. 둘 다 아니면              →  ncs.go.kr 내려받기 링크로 안내
```

**왜 서버가 사이트까지 내보내는가** — 배포된 `https://…github.io` 에서 내 PC 의
`127.0.0.1` 로 붙는 길은 없습니다. Chrome 이 공개 사이트에서 루프백·사설 주소로
나가는 것을 막습니다(Local Network Access). 브라우저에서 직접 확인했습니다 —
요청이 서버에 닿지도 않고(`ERR_BLOCKED_BY_CLIENT`), `local-network-access`
권한은 `denied` 고정이며 `fetch` 의 `targetAddressSpace` 옵션도 먹지 않습니다.
그래서 사이트와 PDF 를 같은 곳에서 내보냅니다. 같은 출처가 되면 막을 것이 없습니다.

| 파일 | 역할 |
|---|---|
| `tools/pdf-server.py` | 로컬 PDF 서버. `127.0.0.1` 전용 · `.pdf` 읽기만 · Range 지원 |
| `assets/modules-pdf.js` | 색인 291건 — 경로·파일명·크기·내려받기 키. **원문은 없음** |
| `assets/pdfview.js` | 모달 뷰어. 폴더 손잡이는 IndexedDB 에만 둔다 |
| `assets/pdfjs/` | pdf.js 4.10.38 (vendored, 1.7MB) — 외부 CDN 을 쓰지 않는다 |

`pdf-server.py` 는 다른 PC 에서 접속할 수 없고(127.0.0.1 바인딩), 지정 폴더 밖이나
`.pdf` 가 아닌 파일은 404 로 거절합니다. 목록도 내보내지 않습니다.
116MB 짜리도 pdf.js 가 필요한 부분만 읽도록 Range 를 지원합니다.

색인 갱신은 `python NCS-CATALOG/scripts/gen/gen_pdf_index.py` 입니다.

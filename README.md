...
원문은 **이 저장소에 없습니다** — 291개 3.0GB(최대 116MB)라 GitHub Pages 한도(1GB)로도
불가능하고, 저작권자 자료라 공개 저장소에 올리지도 않습니다. 원격 링크도 막혀 있습니다:
`ncs.go.kr` 의 내려받기는 `Content-Disposition: attachment` 에 CORS 헤더가 없어
iframe 으로도 pdf.js 로도 띄울 수 없습니다.

그래서 **강사 PC 의 파일을 브라우저가 직접 엽니다.**

```
[PDF] 처음 누름  →  커리큘럼 저장소들을 담은 상위 폴더를 한 번 고름
그 다음부터      →  기억해 둔 폴더에서 바로 열림 (파일은 읽기만 함)
브라우저 재시작   →  [허용] 한 번 (폴더는 그대로 기억)
Chrome/Edge 아님 →  ncs.go.kr 내려받기 링크로 안내
```
